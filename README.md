...
```r
start <- as.Date("01-01-2021",format="%m-%d-%Y")
end   <- as.Date("12-12-2021",format="%m-%d-%Y")
final_data <- rbind()
```
...
```r
final_data %>%   
  filter(Currency %in% c("CNY¥", "EURO€", "IRAN Toman", "PAKISTAN Rs.", "POUND£", "SAUDI RIYAL", "SWISS₣", "UAE DIRHAM", "USD$")) %>%   
  mutate(  
